...
export default function AdminCompilerDashboard() {
  ...
  const sections = useMemo(() => ([
    {
      key: 'overview',
      label: 'Overview',
      caption: 'Health and recent activity',
      to: '/admin/compiler',
    ...
    },
    {
      ...
      caption: 'Author and publish',
      to: '/admin/compiler/create',
      Icon: PlusSquare,
    },
    {
      key: 'management',
      label: 'Problem Management',
      caption: 'Search and open problems',
      to: '/admin/compiler/problems',
      Icon: FileCode2,
    },
    {
      key: 'submissions',
      label: 'Submissions Monitor',
      caption: 'Live judge feed',
      to: '/admin/compiler/submissions',
      Icon: TerminalSquare,
    },
    {
      key: 'analytics',
      label: 'Analytics',
      caption: 'Attempts and performance',
      to: '/admin/compiler/analytics',
      Icon: BarChart3,
    },
  ]), []);
  ...
  const activeSection = pathname.startsWith('/admin/compiler/create')
    ? 'create'
    : pathname.startsWith('/admin/compiler/problems/') || pathname.startsWith('/admin/compiler/problems')
      ? 'management'
      : pathname.startsWith('/admin/compiler/submissions')
        ? 'submissions'
  ...
  const renderContent = () => {
    if (pathname.startsWith('/admin/compiler/create')) return <CreateProblem />;
    if (pathname.startsWith('/admin/compiler/problems/')) return <ProblemDetail />;
    if (pathname.startsWith('/admin/compiler/problems')) return <ProblemManagement />;
    if (pathname.startsWith('/admin/compiler/submissions')) return <SubmissionsMonitor />;
              ...
              <div className="flex flex-wrap gap-2 md:hidden">
                {sections.map((section) => (
                  <Link
                    key={section.key}
                    to={section.to}
                    className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${activeSection === section.key ? 'bg-sky-600 text-white' : 'bg-slate-100 text-slate-600 dark:bg-gray-800 dark:text-gray-300'}`}
                  >
                    {section.label}
                  </Link>
                ))}
