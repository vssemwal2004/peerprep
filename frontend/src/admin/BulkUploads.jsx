...
export default function BulkUploads() {
  ...
  const confirmDelete = async () => {
    if (!deleteTarget || deleteConfirmation !== deleteTarget.name || deletePreview?.blockers?.length) return;
    setWorkingId(deleteTarget._id);
    try {
      const response = await api.deleteBulkUpload(deleteTarget._id, deleteConfirmation);
  ...
  return (
    <div className="min-h-screen bg-slate-50/70 px-4 pb-12 pt-6 dark:bg-gray-950 sm:px-6 lg:px-8">
      ...
      {selected && <><button type="button" aria-label="Close upload details" onClick={() => setSelected(null)} className="fixed inset-0 z-[90] bg-slate-950/35 backdrop-blur-[1px]" /><aside className="fixed right-0 top-0 z-[91] flex h-dvh w-full max-w-md flex-col border-l border-slate-200 bg-white shadow-2xl dark:border-gray-700 dark:bg-gray-900"><div className="flex items-start justify-between border-b border-slate-200 p-5 dark:border-gray-800"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-sky-600">Upload details</p><h2 className="mt-1 text-xl font-bold text-slate-950 dark:text-white">{selected.name}</h2><p className="mt-1 break-all text-xs text-slate-500">{selected.originalFileName}</p></div><button onClick={() => setSelected(null)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-800"><X className="h-4 w-4" /></button></div><div className="flex-1 space-y-5 overflow-y-auto p-5"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-gray-700 dark:bg-gray-800/60"><div className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-emerald-600" /><p className="text-sm font-bold text-slate-900 dark:text-white">Provenance status</p></div><p className="mt-2 text-xs leading-5 text-slate-600 dark:text-gray-300">{selected.provenanceReady ? `${selected.createdRecordCount} created and ${selected.updatedRecordCount} pre-existing records are tracked separately.` : 'Legacy upload: created-record provenance is unavailable, so cascade deletion is disabled.'}</p></div><dl className="grid grid-cols-2 gap-3">{[['Status', selected.status], ['Data type', TYPE_META[selected.entityType]?.label || selected.entityType], ['Total rows', selected.totalRows], ['Tracked records', selected.recordCount], ['Created', selected.createdCount], ['Updated', selected.updatedCount], ['Failed', selected.failedCount], ['Uploaded', new Date(selected.createdAt).toLocaleString()]].map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 p-3 dark:border-gray-700"><dt className="text-[10px] font-bold uppercase tracking-wide text-slate-400">{label}</dt><dd className="mt-1 text-sm font-bold capitalize text-slate-800 dark:text-gray-100">{value ?? 0}</dd></div>)}</dl><div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Uploaded by</p><p className="mt-1 text-sm font-bold text-slate-800 dark:text-white">{selected.uploadedBy?.name || selected.uploadedByEmail}</p><p className="text-xs text-slate-500">{selected.uploadedBy?.email || selected.uploadedByEmail}</p></div></div><div className="grid grid-cols-2 gap-2 border-t border-slate-200 p-5 dark:border-gray-800"><button onClick={() => download(selected, 'data')} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-gray-700 dark:text-gray-200"><Download className="h-4 w-4" />Download data</button><button onClick={() => download(selected, 'errors')} disabled={!selected.failedCount} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-3 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-40 dark:border-gray-700 dark:text-gray-200"><AlertTriangle className="h-4 w-4" />Error report</button></div></aside></>}

      {deleteTarget && <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm"><div role="alertdialog" aria-modal="true" className="w-full max-w-xl rounded-3xl border border-rose-200 bg-white p-6 shadow-2xl dark:border-rose-900/50 dark:bg-gray-900"><div className="flex items-start justify-between gap-4"><div className="flex gap-3"><span className="rounded-xl bg-rose-100 p-2.5 text-rose-600 dark:bg-rose-950/50"><AlertTriangle className="h-6 w-6" /></span><div><h2 className="text-lg font-bold text-slate-950 dark:text-white">Delete upload and created data?</h2><p className="mt-1 text-sm text-slate-500">Pre-existing records that were only updated are never deleted.</p></div></div><button disabled={workingId === deleteTarget._id} onClick={() => setDeleteTarget(null)} className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-gray-800"><X className="h-5 w-5" /></button></div>{!deletePreview ? <div className="py-10 text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-sky-600" /><p className="mt-2 text-xs text-slate-500">Checking linked records…</p></div> : <><div className="mt-5 grid grid-cols-3 gap-2">{[['Created by upload', deletePreview.createdRecords], ['Can be deleted', deletePreview.deletableRecords], ['Shared / retained', deletePreview.retainedSharedRecords]].map(([label, value]) => <div key={label} className="rounded-xl border border-slate-200 p-3 text-center dark:border-gray-700"><p className="text-xl font-bold text-slate-950 dark:text-white">{value}</p><p className="mt-1 text-[10px] font-bold uppercase text-slate-400">{label}</p></div>)}</div>{deletePreview.blockers?.length > 0 && <div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-800 dark:border-rose-900/50 dark:bg-rose-950/25 dark:text-rose-200"><p className="font-bold">Deletion is blocked</p><ul className="mt-2 list-disc space-y-1 pl-5 text-xs">{deletePreview.blockers.map((blocker) => <li key={blocker.code}>{blocker.message}</li>)}</ul></div>}{deletePreview.warnings?.length > 0 && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/25 dark:text-amber-200">{deletePreview.warnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}<label className="mt-5 block"><span className="mb-1.5 block text-xs font-bold text-slate-700 dark:text-gray-300">Type <strong>{deleteTarget.name}</strong> to confirm</span><input autoFocus disabled={deletePreview.blockers?.length > 0} value={deleteConfirmation} onChange={(event) => setDeleteConfirmation(event.target.value)} className="h-11 w-full rounded-xl border border-slate-300 px-3 text-sm font-bold outline-none focus:border-rose-500 disabled:bg-slate-100 dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></label><div className="mt-6 flex justify-end gap-2"><button onClick={() => setDeleteTarget(null)} className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-700 dark:border-gray-700 dark:text-gray-200">Cancel</button><button disabled={deleteConfirmation !== deleteTarget.name || deletePreview.blockers?.length > 0 || workingId === deleteTarget._id} onClick={confirmDelete} className="inline-flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-40">{workingId === deleteTarget._id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}Delete created data</button></div></>}</div></div>}
    </div>
  );
}
