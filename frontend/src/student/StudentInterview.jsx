...
export default function StudentInterview() {
  ...
  const StatsComponent = () => {
    ...
    const joinedCount = filteredEvents.filter((event) => event.joined).length;
    const upcomingCount = filteredEvents.filter((event) => {
      if (!event.joined) return false;
      ...
      return (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-slate-200 dark:border-gray-700 p-3">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-base font-semibold text-slate-800 dark:text-gray-100">{value}</div>
              <div className="text-[11px] text-slate-600 dark:text-gray-400">{label}</div>
            </div>
            <div className={`p-1.5 rounded ${toneClasses}`}>
    ...
    return (
      <div className="grid grid-cols-3 gap-2">
        <StatCard label="Total" value={filteredEvents.length} Icon={BookOpen} tone="sky" />
        <StatCard label="Joined" value={joinedCount} Icon={CheckCircle} tone="emerald" />
        <StatCard label="Upcoming" value={upcomingCount} Icon={Calendar} tone="amber" />
      </div>
    );
  };
  ...
  const EventList = () => (
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-slate-200 dark:border-gray-700 p-3 sm:p-4 h-[calc(100vh-5rem)] sm:h-[calc(100vh-4rem)] flex flex-col overflow-hidden">
      ...
      {selectedEvent ? (
        ...
        <>
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h2 className="text-base sm:text-lg font-semibold text-slate-800 dark:text-gray-100">Your Interviews</h2>
  ...
  const Placeholder = () => {
    const now = Date.now();
    const upcomingPairs = pairs
      .filter((p) => p.scheduledAt && new Date(p.scheduledAt).getTime() > now)
      .sort((a, b) => new Date(a.scheduledAt) - new Date(b.scheduledAt));
    const nextInterview = upcomingPairs[0] || null;
    const nextRole = nextInterview ? getUserRoleInPair(nextInterview) : null;
    const partner =
      nextRole === "interviewer"
        ? nextInterview?.interviewee
        : nextRole === "interviewee"
        ? nextInterview?.interviewer
        : null;
    const partnerName = partner?.name || partner?.email || "Partner";
    const meetingReady = nextInterview?.scheduledAt
      ? Date.now() >= new Date(nextInterview.scheduledAt).getTime() - 30 * 60 * 1000
      : Boolean(nextInterview?.meetingLink);
    const activePair = selectedPair || nextInterview;
    const activeScheduledAt = activePair?.scheduledAt ? new Date(activePair.scheduledAt).getTime() : null;
    const feedbackPending =
      activePair?.status === "scheduled" && activeScheduledAt && activeScheduledAt <= Date.now()
        ? "Yes"
        : activePair?.status === "completed"
        ? "Submitted"
        : "No";

    const joinedEvents = events.filter((event) => event.joined);
    const getEventStatus = (event) => {
      const eventPairs = pairs.filter((p) => p.event?._id === event._id);
      const hasCompleted =
        eventPairs.some((p) => p.status === "completed") ||
        (event.endDate && new Date(event.endDate).getTime() < now);
      const hasScheduled = eventPairs.some((p) => p.status === "scheduled");
      if (hasCompleted) return "Completed";
      if (hasScheduled) return "Scheduled";
      return "Pending";
    };

    const handleActiveCTA = () => {
      if (nextInterview?.meetingLink && meetingReady) {
        window.open(nextInterview.meetingLink, "_blank", "noopener,noreferrer");
        return;
      }
      if (nextInterview?.event) {
        handleEventClick(nextInterview.event);
      }
    };
    ...
    return (
      <div className="flex-1 flex flex-col p-4 sm:p-6 lg:p-8 overflow-y-auto">
        <div className="mb-6">
          <h1 className="text-2xl sm:text-3xl font-semibold text-slate-800 dark:text-gray-100 mb-1.5">
            Interview Sessions
          </h1>
          <p className="text-slate-500 dark:text-gray-400 text-sm sm:text-base">
            Manage your interviews, schedule sessions, and track progress.
          </p>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          <div className="lg:col-span-2 rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="text-xs uppercase tracking-wider text-slate-400">Active Interview</div>
                <h2 className="text-sm font-semibold text-slate-800 dark:text-gray-100 mt-1">
                  {nextInterview ? nextInterview.event?.name || "Upcoming Session" : "No upcoming session"}
                </h2>
              </div>
              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 dark:bg-gray-700 text-slate-600 dark:text-gray-300">
            ...
            <div className="space-y-2 text-sm text-slate-600 dark:text-gray-300">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium text-slate-800 dark:text-gray-100">
                  {nextInterview?.scheduledAt ? fmt(nextInterview.scheduledAt) : "Not scheduled"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span>Partner</span>
                <span className="font-medium text-slate-800 dark:text-gray-100">{partnerName}</span>
              </div>
            </div>
            <button
              onClick={handleActiveCTA}
              disabled={!nextInterview}
              className={`mt-4 w-full sm:w-auto px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${
                nextInterview
          ...
          <div className="rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            <div className="text-xs uppercase tracking-wider text-slate-400 mb-3">Session Status</div>
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-gray-400">Pair status</span>
                <span className="font-medium text-slate-800 dark:text-gray-100">
                  {activePair?.status || "Not paired"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-gray-400">Slot confirmation</span>
                <span className="font-medium text-slate-800 dark:text-gray-100">
                  {activePair?.scheduledAt ? "Confirmed" : "Awaiting"}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-500 dark:text-gray-400">Feedback pending</span>
                <span className="font-medium text-slate-800 dark:text-gray-100">{feedbackPending}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-semibold text-slate-800 dark:text-gray-100">Your Interviews</h3>
            <span className="text-xs text-slate-400">{joinedEvents.length} joined</span>
          </div>
          {joinedEvents.length === 0 ? (
            <div className="text-sm text-slate-500 dark:text-gray-400">
              No interviews joined yet. Select an event from the sidebar to get started.
            </div>
          ) : (
            <div className="space-y-3">
              {joinedEvents.map((event) => {
                ...
                return (
                  <div
                    key={event._id}
                    className="rounded-lg border border-slate-200 dark:border-gray-700 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3"
                  >
                    <div>
                      <div className="font-medium text-slate-800 dark:text-gray-100">{event.name}</div>
                      <div className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${statusColor}`}>
                        {status}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleEventClick(event)}
                        className="px-3 py-1.5 text-xs font-semibold rounded-lg border border-slate-200 dark:border-gray-700 text-slate-600 dark:text-gray-300 hover:bg-slate-50 dark:hover:bg-gray-700"
                      >
                        View details
                      </button>
                      <button
              ...
              {sidebarExpanded ? (
                <>
                  <div className="flex items-center gap-2 min-w-0">
                    <h2 className="text-base font-semibold text-slate-800 dark:text-gray-100 truncate">Your Interviews</h2>
