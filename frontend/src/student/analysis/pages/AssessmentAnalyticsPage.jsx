...
export default function AssessmentAnalyticsPage({ analytics, assessmentMovement }) {
  ...
  const hasAttempts = Number(assessments.attempts || 0) > 0;
  ...
  return (
    <div className="space-y-4">
      <ActionCard
        title={assessments.integrityScore < 85 && hasAttempts ? "Improve assessment reliability" : "Review before the next timed attempt"}
        reason={insight?.summary || "Assessment guidance appears after a submitted test."}
        action={insight?.action}
        tone={assessments.integrityScore < 85 && hasAttempts ? "amber" : "sky"}
      />

      <MetricGrid columns={5}>
        <MetricTile label="Final attempts" value={assessments.attempts || 0} helper="Submitted assessment records" Icon={Gauge} tone="sky" available={hasAttempts} />
        <MetricTile label="Adjusted average" value={Math.round(adjustedAverage)} suffix="%" helper="Reliability-adjusted score" Icon={TrendingUp} tone={getScoreTone(adjustedAverage)} available={hasAttempts} />
        <MetricTile label="Latest score" value={Math.round(assessments.latestAdjustedScore || 0)} suffix="%" helper="Latest adjusted result" Icon={Award} tone={getScoreTone(assessments.latestAdjustedScore)} available={hasAttempts} />
        <MetricTile label="Reliability" value={Math.round(assessments.integrityScore ?? 0)} suffix="%" helper={`${assessments.securityRisk || "low"} risk`} Icon={ShieldCheck} tone={getScoreTone(assessments.integrityScore)} available={hasAttempts} />
        <MetricTile label="Average time" value={formatDuration(assessments.avgTimeTakenSec)} helper="Per submitted attempt" Icon={Clock3} tone="slate" available={hasAttempts && Number(assessments.avgTimeTakenSec || 0) > 0} />
      </MetricGrid>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        ...
        <Panel className="p-5">
          ...
          <div className="mt-6 space-y-6">
            <ScoreBar label="Score stability" value={assessments.stabilityScore || 0} available={hasAttempts} helper="Variation across recent adjusted scores" />
            <ScoreBar label="Assessment reliability" value={assessments.integrityScore ?? 0} available={hasAttempts} helper={`${assessments.violationAttempts || 0} flagged attempt${assessments.violationAttempts === 1 ? "" : "s"}`} />
            <ScoreBar label="Best score" value={assessments.highestScore || 0} available={hasAttempts} helper="Highest raw result" tone="emerald" />
