...
Coordinator accounts also have an administrator-controlled data scope:

- **Own data** restricts enabled features to students assigned to the coordinator and records created/owned by that coordinator.
- **Whole data** allows institution-wide visibility inside enabled features. It does not grant a disabled feature; feature permissions and data scope are evaluated together.

The scope can be selected during individual coordinator creation and changed later in **Coordinator Access**. New coordinators default to the safer **Own data** scope. Teacher IDs are generated by the server as unique five-digit integers from `10000` to `99999`; administrators do not enter them manually. Existing legacy alphanumeric IDs remain supported so linked student and learning records continue to work. Department and college values must come from active academic master data.

Administrative list views retain audit attribution for coordinator/admin-created content. Question-library rows expose an **Added by** identity popup, while saved bulk-student lists expose an **Uploaded by** identity popup. These show the actor's name, role, email, and Teacher ID when applicable. Attribution is informational and does not replace backend permission and data-scope enforcement.
