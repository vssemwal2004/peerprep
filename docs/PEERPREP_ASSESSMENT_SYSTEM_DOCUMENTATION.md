...
The scope can be selected during individual coordinator creation and changed later in **Coordinator Access**. New coordinators default to the safer **Own data** scope. Teacher IDs are generated by the server as unique five-character uppercase alphanumeric codes; administrators do not enter them manually. Department and college values must come from active academic master data.
